...
def search(argv):
	...
	if len(argv) == 1:
		...
		form = "%Y-%m-%d-%H-%M"
		_to = dt.datetime.strptime(argv[1], form)

	_from = _to - dt.timedelta(10)
	...
	vfd_table = '345---------001BMSHVAFANFDB---VAL001'
	pum_table = '345---------001BMSWATSECPUM---VAL001'
	wsp_table = '345---------001BMSWATSEC------VAL001'
	...
	vfd_rec_table = '345---------001TPOHVAFANFDB---VAL001'
	wsp_rec_table = '345---------001TPOWATSEC------VAL001'
	...
	s += bms_interface.get_bms_series(vfd_table, _from, _to, _group='vfd', _domain=(0, 100))
	...
	while t < ndf.pointSize:
		...
		for i1 in odf.group['fan'] + odf.group['pump'] + odf.group['vfd'] + odf.group['wsp']:
	...
	for g in ['fan', 'pump', 'wsp', 'vfd']:

		table = g + 'fan_rec_table'
	...
	for g in ['electric', 'temp', 'steam', 'occupancy']:
