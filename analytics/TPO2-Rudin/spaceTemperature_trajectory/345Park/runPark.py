...
_module = 'park_spaceTemperature_trajectory'
...
def main(argv):
    ...
    try:
        trajectoryPredictor = spaceTempTrajectory.trajectoryPredictorSptPark(conn, cursor, paramObj, 14, now, 1, performGridSearch,
    ...
    for timestep in range(2,9):
        try:
            trajectoryPredictorKeyTimestep[timestep] = spaceTempTrajectory.trajectoryPredictorSptPark(conn, cursor, paramObj,
                                                                        14, now, timestep, performGridSearch,
                                                                        [8, 4, 2, 1], relevantHours, os.path.dirname(os.path.abspath(__file__))+'\\')
            trajectoryPredictorKeyTimestep[timestep].borrowData(trajectoryPredictor.SatDataCollector,
                                                              trajectoryPredictor.SptDataCollector,
                                                              trajectoryPredictor.SptPredictionsDataCollector,
                                                              trajectoryPredictor.SatSPDataCollector)
