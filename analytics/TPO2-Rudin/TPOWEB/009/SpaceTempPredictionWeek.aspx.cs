...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_009_SpaceTempPredictionWeek : System.Web.UI.Page
    {
        ...
        private void PlotChart(string sensorid, string sensorname)
        {
            ...
            dbQueryStartUpTime = "SELECT 80 AS Value, TIMESTAMP FROM [009_Prediction_Start_Up_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            dbQueryRampDownTime = "SELECT 65 AS Value, TIMESTAMP FROM [009_Prediction_Ramp_Down_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [355].dbo.[355---------000TPOFORTEMSPA001---001_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND (Floor = '" + fl + "') AND (Quadrant = '" + qd + "')  ORDER BY [Prediction_Date]";
            ...
            //arrow for nowcast
            dbQueryStringArrow = "SELECT  [TIMESTAMP], [Value] FROM [dbo].[009_Prediction_Space_Temperature_" + sensorname + "_Start] UNION SELECT  [TIMESTAMP], [Value] FROM [dbo].[009_Prediction_Space_Temperature_" + sensorname + "_End] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
