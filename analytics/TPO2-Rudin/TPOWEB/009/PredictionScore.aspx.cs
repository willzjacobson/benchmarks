...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_009_PredictionScore : System.Web.UI.Page
    {
        ...
        private void PlotScoreChart()
        {
            ...
            dbQueryStringme = "SELECT [Prediction_Date] AS TIMESTAMP, [RMSE] AS Value FROM [355].dbo.[355---------000TPOFORELECON001---001_stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, [Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY [Prediction_Date]";

            dbQueryStringmo = "SELECT [Prediction_Date] AS TIMESTAMP, [RMSE] AS Value FROM [355].dbo.[355---------000TPOFORPEOBUI001---001_stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, [Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY [Prediction_Date]";
        ...
        private void PlotTempScoreChart(string sensorid, string sensorname, int i)
        {
            ...
            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [RMSE] AS Value FROM [355].dbo.[355---------000TPOFORTEMSPA002---001_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, [Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1)  AND (Floor = '" + fl + "' AND Quadrant  = '" + qd + "') ORDER BY [Prediction_Date]";
