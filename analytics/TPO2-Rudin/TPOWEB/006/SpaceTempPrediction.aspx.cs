...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_006_SpaceTempPrediction : System.Web.UI.Page
    {
        ...
        private void PlotChart(string sensorid, string sensorname, int i)
        {
            ...
            //chart series    
            dbQueryStringSAT = "SELECT Value, TIMESTAMP FROM [WHI].dbo.[WHI---------006BMSHVASFASAT---VAL001] WHERE (EQUIPMENT_NO = '001') AND (ZONE = 'Z" + sensorSATid + "') AND (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            //SAT set-points
            dbQueryStringSATSP = "SELECT Value, TIMESTAMP FROM [WHI].dbo.[WHI---------006BMSHVASFASAT---STP001]  WHERE (EQUIPMENT_NO = '001') AND (ZONE = 'Z" + sensorSATid + "') AND (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
