...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_KPMG_SpaceTempPredictionTenant : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, 'Prediction_Space_Temperature_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%Space_Temperature%' AND TABLE_NAME IN ('Prediction_Space_Temperature_13_SW', 'Prediction_Space_Temperature_2_NW', 'Prediction_Space_Temperature_4_NW','Prediction_Space_Temperature_38_NW','Prediction_Space_Temperature_40_NE') ORDER BY TABLE_NAME";
        ...
        private void PlotChart(string sensorid, string sensorname)
        {
            ...
            dbQueryStringA = "SELECT TIMESTAMP, Value as Actual FROM [RUDINSERVER_FL" + sensorname + "_SPACETEMP] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            {
                case "13_SW":
                    sensorSATid = "RUDINSERVER_S9_SAT";
                    break;
                case "18_SE":
                    sensorSATid = "RUDINSERVER_S11_SAT";
                    break;
                case "2_NW":
                    sensorSATid = "RUDINSERVER_S10_SAT";
                    break;
                case "20_SW":
                    sensorSATid = "RUDINSERVER_S4_SAT";
                    break;
                case "24_SE":
                    sensorSATid = "RUDINSERVER_S6_SAT";
                    break;
                case "32_NW":
                    sensorSATid = "RUDINSERVER_S4_SAT";
                    break;
                case "38_NW":
                    sensorSATid = "RUDINSERVER_S3_SAT";
                    break;
                case "40_NE":
                    sensorSATid = "RUDINSERVER_S5_SAT";
                    break;
                case "5_SE":
                    sensorSATid = "RUDINSERVER_S12_SAT";
                    break;
                default:
                    sensorSATid = "RUDINSERVER_S9_SAT";
                    break;
            ...
            //chart series    
            dbQueryStringSAT = "SELECT Value, TIMESTAMP FROM [" + sensorSATid + "] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            s.LegendText = sensorSATid.Substring(12);
            s.ToolTip = s.LegendText;

            Chart1.Series.Add(s);

            //SAT set-points
            dbQueryStringSATSP = "SELECT Value, TIMESTAMP FROM [" + sensorSATid + "_SP] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            s.Points.DataBindXY(myReader, "TIMESTAMP", myReader, "Value");
            s.LegendText = sensorSATid.Substring(12)+"_SP";
            s.ToolTip = s.LegendText;

            Chart1.Series.Add(s);


            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [Space_Temp_Prediction_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND Floor = LEFT('" + sensorname + "', CHARINDEX('_', '" + sensorname + "')-1) ORDER BY [Prediction_Date]";
            ...
            //arrow for nowcast
            dbQueryStringArrow = "SELECT  [TIMESTAMP], [Value] FROM [Rudin_345Park].[dbo].[Prediction_Space_Temperature_" + sensorname + "_Start] UNION SELECT  [TIMESTAMP], [Value] FROM [Rudin_345Park].[dbo].[Prediction_Space_Temperature_" + sensorname + "_End] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
