...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_004_PredictionScoreAvg : System.Web.UI.Page
    {
        ...
        private void PlotScoreChart()
        {
            ...
            dbQueryStringme = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [1BP].dbo.[1BP---------000TPOFORELECON001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [1BP].dbo.[1BP---------000TPOFORELECON001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";

            dbQueryStringmo = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [1BP].dbo.[1BP---------000TPOFORPEOBUI001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [1BP].dbo.[1BP---------000TPOFORPEOBUI001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";
        ...
        private void PlotTempScoreChart(string sensorid, string sensorname, int i)
        {
            ...
            dbQueryStringm = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [1BP].dbo.[1BP---------000TPOFORTEMSPA001---001_Stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date] AND (Floor = '" + fl + "' AND Quadrant  = '" + qd + "')) AS Value_AVG FROM [1BP].dbo.[1BP---------000TPOFORTEMSPA001---001_Stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) AND (Floor = '" + fl + "' AND Quadrant  = '" + qd + "') ORDER BY b.[Prediction_Date]";
