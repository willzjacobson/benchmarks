...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_005_ElectricityPrediction : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string sensorid = "";
            string sensorname = "";
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '005_Prediction_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%005_Prediction_ELECTRICITY%' ORDER BY TABLE_NAME";
            ...
            dbQueryStringa = "SELECT Value AS Actual, TIMESTAMP FROM [005_ALEVELMETERALLKW] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') AND Value < 5000.0 ORDER BY TIMESTAMP";

            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [641].dbo.[641---------000TPOFORELECON001---001_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') ORDER BY [Prediction_Date]";
