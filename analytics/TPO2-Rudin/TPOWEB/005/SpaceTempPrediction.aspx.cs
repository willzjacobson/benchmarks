...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_005_SpaceTempPrediction : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                DateFrom.Text = DateTime.Now.AddDays(-1).ToShortDateString();
        ...
        private void FillData()
        {
            ...
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '005_Prediction_Space_Temperature_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%005_Prediction_Space_Temperature%' ORDER BY TABLE_NAME";
            ...
            foreach (ListItem sti in SensorTypeList.Items)
            {
                if (sti.Selected)
                {
                    ...
                    PlotChart(sensorid, sensorname, i);
        ...
        private void PlotTime()
        {
            ...
            dbQueryStartUpTime = "SELECT 80 AS Value, TIMESTAMP FROM [005_Prediction_Start_Up_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            dbQueryRampDownTime = "SELECT 65 AS Value, TIMESTAMP FROM [005_Prediction_Ramp_Down_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
        ...
        private void PlotChart(string sensorid, string sensorname, int i)
        {
            ...
            dbQueryStringA = "SELECT TIMESTAMP, Value as Actual FROM [005_Actual_Space_Temperature_" + sensorname + "] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            if (ShowBounds.Checked)
            {
                ...
                s_upper = new Series("Series_upper" + i);
            ...
            if (ShowBounds2.Checked)
            {
                ...
                myCommand.Connection.Open();
                myReader = myCommand.ExecuteReader(CommandBehavior.Default);
                s_upper = new Series("Series_upper2" + i);
                s_upper.ChartType = SeriesChartType.SplineRange;
                s_upper.YValuesPerPoint = 2;
            ...
            //chart series    
            dbQueryStringSAT = "SELECT Value, TIMESTAMP FROM [641].dbo.[641---------005BMSHVASFASAT---VAL001] WHERE (EQUIPMENT_NO = '001') AND (FLOOR = 'F" + sensorSATid + "') AND (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            //SAT set-points
            dbQueryStringSATSP = "SELECT Value, TIMESTAMP FROM [641].dbo.[641---------005BMSHVASFASAT---STP001] WHERE (EQUIPMENT_NO = '001') AND (FLOOR = 'F" + sensorSATid + "') AND (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            if (ShowSAT.Checked)
            {
                ...
                s = new Series("Series_SAT" + i + 1);
                s.ChartType = SeriesChartType.FastLine;
                s.ShadowColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
                s.BorderColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
                s.Points.DataBindXY(myReader, "TIMESTAMP", myReader, "Value");
                s.LegendText = "F" + sensorSATid + "_SAT";
                ...
                myCommand.Connection.Open();
                myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection);
                s = new Series("Series_SAT" + i + 2);
                s.ChartType = SeriesChartType.FastLine;
                s.ShadowColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
                s.BorderColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
                s.Color = System.Drawing.ColorTranslator.FromHtml("#989898");
                s.Points.DataBindXY(myReader, "TIMESTAMP", myReader, "Value");
                s.LegendText = "F" + sensorSATid + "_SAT_SP";
                s.ToolTip = s.LegendText;
            ...
            //arrow for nowcast
            dbQueryStringArrow = "SELECT  [TIMESTAMP], [Value] FROM [dbo].[005_Prediction_Space_Temperature_" + sensorname + "_Start] UNION SELECT  [TIMESTAMP], [Value] FROM [dbo].[005_Prediction_Space_Temperature_" + sensorname + "_End] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
