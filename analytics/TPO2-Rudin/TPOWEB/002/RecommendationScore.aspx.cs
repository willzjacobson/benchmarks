...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_002_RecommendationScore : System.Web.UI.Page
    {
        ...
        private void PlotScoreChart()
        {
            ...
            dbQueryStringms = "SELECT [Actual_DateTime] AS TIMESTAMP, [Agreed]*100 AS Value FROM [560].dbo.[Score_Startup_Rampdown] b WHERE (b.[Actual_DateTime] > '" + fromDate.ToString() + "') AND (b.Actual_DateTime < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Actual_DateTime]) + @@DATEFIRST) % 7) NOT IN (0, 1) AND [Tag] = 'startup' AND Building = '560' and [Agreed] is not null and [Actual_DateTime] is not null ORDER BY b.[Actual_DateTime]";

            dbQueryStringmr = "SELECT [Actual_DateTime] AS TIMESTAMP, [Agreed]*100 AS Value FROM [560].dbo.[Score_Startup_Rampdown] b WHERE (b.[Actual_DateTime] > '" + fromDate.ToString() + "') AND (b.Actual_DateTime < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Actual_DateTime]) + @@DATEFIRST) % 7) NOT IN (0, 1) AND [Tag] = 'rampdown' AND Building = '560' and [Agreed] is not null and [Actual_DateTime] is not null ORDER BY b.[Actual_DateTime]";
