...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_002_PredictionScoreAvg : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '002_Prediction_Space_Temperature_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%002_Prediction_Space_Temperature%' ORDER BY TABLE_NAME";
        ...
        private void PlotScoreChart()
        {
            ...
            dbQueryStringme = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [560].dbo.[560---------000TPOFORELECON001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [560].dbo.[560---------000TPOFORELECON001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";

            dbQueryStringmo = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [560].dbo.[560---------000TPOFORPEOBUI001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [560].dbo.[560---------000TPOFORPEOBUI001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";
        ...
        private void PlotTempScoreChart(string sensorid, string sensorname, int i)
        {
            ...
            dbQueryStringm = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [560].dbo.[560---------000TPOFORTEMSPA001---001_Stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date] AND (Floor = 'F" + sensorname + "')) AS Value_AVG FROM [560].dbo.[560---------000TPOFORTEMSPA001---001_Stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) AND (Floor = 'F" + sensorname + "') ORDER BY b.[Prediction_Date]";
