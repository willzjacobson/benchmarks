...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_003_PredictionScoreAvg : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '003_Prediction_Space_Temperature_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%003_Prediction_Space_Temperature%' ORDER BY TABLE_NAME";
        ...
        private void PlotScoreChart()
        {
            ...
            dbQueryStringme = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [4E5].dbo.[4E5---------000TPOFORELECON001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [4E5].dbo.[4E5---------000TPOFORELECON001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";

            dbQueryStringms = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [4E5].dbo.[4E5---------000TPOFORSTECON001---001_Stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [4E5].dbo.[4E5---------000TPOFORSTECON001---001_Stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";

            dbQueryStringmo = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [4E5].dbo.[4E5---------000TPOFORPEOBUI001---001_stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date]) AS Value_AVG FROM [4E5].dbo.[4E5---------000TPOFORPEOBUI001---001_stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) ORDER BY b.[Prediction_Date]";
        ...
        private void PlotTempScoreChart(string sensorid, string sensorname, int i)
        {
            ...
            string qd = "C" + sensorname.Substring((sensorname.IndexOf("_") + 1)) + "O"; 
            Series m;
            ...
            dbQueryStringA = "SELECT TIMESTAMP, Value as Actual FROM [003_Actual_Space_Temperature_" + sensorname + "] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";


            dbQueryStringm = "SELECT Prediction_Date AS TIMESTAMP, RMSE AS Value, (SELECT AVG(a.[RMSE]) FROM [4E5].dbo.[4E5---------000TPOFORTEMSPA001---001_Stats] a WHERE a.[Prediction_Date] >= DATEADD(dd,-14,b.[Prediction_Date]) AND a.[Prediction_Date] < b.[Prediction_Date] AND (Floor = '" + fl + "' AND Quadrant  = '" + qd + "')) AS Value_AVG FROM [4E5].dbo.[4E5---------000TPOFORTEMSPA001---001_Stats] b WHERE (b.[Prediction_Date] > '" + fromDate.ToString() + "') AND (b.Prediction_Date < '" + toDate.ToString() + "') AND ((DATEPART(dw, b.[Prediction_Date]) + @@DATEFIRST) % 7) NOT IN (0, 1) AND (Floor = '" + fl + "' AND Quadrant  = '" + qd + "') ORDER BY b.[Prediction_Date]";
