...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_003_SpaceTempPredictionWeek : System.Web.UI.Page
    {
        ...
        private void PlotChart(string sensorid, string sensorname)
        {
            ...
            dbQueryStartUpTime = "SELECT 80 AS Value, TIMESTAMP FROM [003_Prediction_Start_Up_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            dbQueryRampDownTime = "SELECT 65 AS Value, TIMESTAMP FROM [003_Prediction_Ramp_Down_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            if (sensorname.Substring(1,1).Equals("_"))
            { 
                fl = "F0" + sensorname.Substring(0, 1);
                qd = "C" + sensorname.Substring(2, 1) + "O";
            }
            else
            { 
                fl = "F" + sensorname.Substring(0, 2);
                qd = "C" + sensorname.Substring(3, 1) + "O";
            }


            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [4E5].dbo.[4E5---------000TPOFORTEMSPA001---001_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') AND (Floor = '" + fl + "') AND (Quadrant = '" + qd + "')  ORDER BY [Prediction_Date]";
            ...
            //arrow for nowcast
            dbQueryStringArrow = "SELECT  [TIMESTAMP], [Value] FROM [dbo].[003_Prediction_Space_Temperature_" + sensorname + "_Start] UNION SELECT  [TIMESTAMP], [Value] FROM [dbo].[003_Prediction_Space_Temperature_" + sensorname + "_End] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
