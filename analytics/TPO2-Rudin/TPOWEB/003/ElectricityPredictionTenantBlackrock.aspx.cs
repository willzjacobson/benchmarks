...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_003_ElectricityPredictionTenantBlackrock : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string sensorid = "";
            string sensorname = "";
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '003_Prediction_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%003_Prediction_ELEC_Blackrock%' ORDER BY TABLE_NAME";
            ...
            dbQueryStringa = "SELECT Usage_Kw AS Actual, TIMESTAMP FROM [4E5].dbo.[4E5---------000TPOFORELETEC001---001_Tenant_load] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";

            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [4E5].dbo.[4E5---------000TPOFORELETEC001---001_stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') ORDER BY [Prediction_Date]";
