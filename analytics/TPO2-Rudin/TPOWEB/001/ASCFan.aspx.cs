...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_001_ASCFan : System.Web.UI.Page
    {
        ...
        private void PlotChart(string sensorid, string sensorname)
        {
            ...
            dbQueryPreheatTime = "SELECT 85 AS Value, TIMESTAMP FROM [001_Prediction_Preheat_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            string queryEquipment = "SELECT DISTINCT EQUIPMENT_NO FROM [345].[dbo].[345---------001TPOHVAFANLCP---VAL001] ORDER BY EQUIPMENT_NO";
            ...
            foreach (DataRow data in dsEquipment.Tables[0].Rows)
            {
                ...
                //dbQueryStringm = "SELECT [TIMESTAMP],([VALUE]+" + count +") AS Value FROM [345].[dbo].[345---------001TPOHVAFANLCP---VAL001] WHERE EQUIPMENT_NO= '" +data[0]+"' AND ([TIMESTAMP] > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "')  ORDER BY [TIMESTAMP]";

                dbQueryStringm = "SELECT [TIMESTAMP],([VALUE]+" + count + ") AS Value FROM [345].[dbo].[345---------001TPOHVAFANLCP---VAL001] WHERE EQUIPMENT_NO = '" + data[0] + "' AND [RUNTIME] = (SELECT MAX(RUNTIME) FROM [345].[dbo].[345---------001TPOHVAFANLCP---VAL001])  ORDER BY [TIMESTAMP]";

                myCommand = new SqlCommand(dbQueryStringm, dbConnection);
                myCommand.Connection.Open();
                myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection);
                m = new Series("Series" + 3 + count);
            ...
            foreach (DataRow data in dsPump.Tables[0].Rows)
            {
                ...
                //dbQueryStringm = "SELECT [TIMESTAMP],([VALUE]+" + ct + ") AS Value FROM [345].[dbo].[345---------001TPOWATSECPUM---VAL001] WHERE EQUIPMENT_NO= '" + data[0] + "' AND ([TIMESTAMP] > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "')  ORDER BY [TIMESTAMP]";

                dbQueryStringm = "SELECT [TIMESTAMP],([VALUE]+" + ct + ") AS Value FROM [345].[dbo].[345---------001TPOWATSECPUM---VAL001] WHERE EQUIPMENT_NO= '" + data[0] + "' AND [RUNTIME] = (SELECT MAX(RUNTIME) FROM [345].[dbo].[345---------001TPOWATSECPUM---VAL001])  ORDER BY [TIMESTAMP]";
