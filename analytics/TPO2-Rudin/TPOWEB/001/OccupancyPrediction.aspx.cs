...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_001_OccupancyPrediction : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string sensorid = "";
            string sensorname = "";
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '001_Prediction_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%001_Prediction_OCCUPANCY%' ORDER BY TABLE_NAME";
            DataSet dsSensor = new DataSet();
            string dbQueryString;
            string dbQueryStringa;
            string dbQueryStringm;
            string dbQueryStringv;
            ...
            dbQueryStringa = "SELECT Value AS Actual, TIMESTAMP FROM [345].[dbo].[345---------000SECCNTPEOBUI---VAL001] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') AND Value < 10000.0 ORDER BY TIMESTAMP";
            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [345].dbo.[345---------000TPOFORPEOBUI001---001_stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') ORDER BY [Prediction_Date]";

            dbQueryStringv = "SELECT [trigger_tm], (([vfd_action]*(-1)+1)*500+500) AS ACTION FROM [345].[dbo].[occupancy_vfd_triggers] WHERE (trigger_tm > '" + fromDate.ToString() + "') AND (trigger_tm < '" + toDate.ToString() + "') ORDER BY trigger_tm";
            ...
            myCommand = new SqlCommand(dbQueryStringv, dbConnection);
            myCommand.Connection.Open();
            myReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection);
            v = new Series("Series" + 4);
            v.ChartType = SeriesChartType.RangeColumn;
            v.ShadowColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
            v.BorderColor = System.Drawing.ColorTranslator.FromHtml("#FFCC66");
            v.Color = System.Drawing.ColorTranslator.FromHtml("#00FF00");
            v.Points.DataBindXY(myReader, "trigger_tm", myReader, "ACTION");
            v.LegendText = "VFD Rampdown Action";
            Chart1.Series.Add(v);
