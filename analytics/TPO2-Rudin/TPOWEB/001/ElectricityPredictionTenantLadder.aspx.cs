...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_001_ElectricityPredictionTenantLadder : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string sensorid = "";
            string sensorname = "";
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '001_Prediction_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%001_Prediction_ELEC_Ladder%' ORDER BY TABLE_NAME";
            ...
            dbQueryStringa = "SELECT Usage_Kw AS Actual, TIMESTAMP FROM [345].dbo.[345---------000TPOFORELETEC004---001_Tenant_load] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [MAPE] AS Value FROM [345].dbo.[345---------000TPOFORELETEC004---001_stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') ORDER BY [Prediction_Date]";
