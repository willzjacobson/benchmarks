...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_001_ASC_Output_Temp : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string queryRuntime = "SELECT DISTINCT [RUNTIME] FROM [345].[dbo].[asc_output_temp] ORDER BY [RUNTIME] DESC";
                ...
                string querySensor = "SELECT DISTINCT Floor + '_' +  Quadrant AS SHORT_NAME FROM [345].[dbo].[asc_output_temp] ORDER BY Floor + '_' +  Quadrant";
        ...
        private void PlotChart(string runtime, string sensorid, int i)
        {
            ...
            Series r;
                ...
                dbQueryStringm = "SELECT [TIMESTAMP],[VALUE] AS Value FROM [345].[dbo].[asc_output_temp] WHERE Floor + '_' +  Quadrant = '" + sensorid+ "' AND [RUNTIME] = '" + runtime + "'  ORDER BY [TIMESTAMP]";
                dbQueryStringr = "SELECT [TIMESTAMP],[VALUE] AS Value FROM  [345].dbo.[345---------001BMSHVATEMSPA---VAL001] WHERE Floor + '_' +  Quadrant = '" + sensorid + "' AND [TIMESTAMP] >= DATEADD(hour,-12,(SELECT MIN([TIMESTAMP]) FROM [345].[dbo].[asc_output_temp] WHERE Floor + '_' +  Quadrant = '" + sensorid + "' AND [RUNTIME] = '" + runtime + "')) AND [TIMESTAMP] <= (SELECT MAX([TIMESTAMP]) FROM [345].[dbo].[asc_output_temp] WHERE Floor + '_' +  Quadrant = '" + sensorid + "' AND [RUNTIME] = '" + runtime + "')  ORDER BY [TIMESTAMP]";
