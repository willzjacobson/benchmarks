...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_001_ASC_Output_Occupancy : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string queryRuntime = "SELECT DISTINCT [RUNTIME] FROM [345].[dbo].[asc_output_occupancy] ORDER BY [RUNTIME] DESC";
                ...
                string querySensor = "SELECT DISTINCT EQUIPMENT_NO AS SHORT_NAME FROM [345].[dbo].[asc_output_occupancy] ORDER BY EQUIPMENT_NO";
        ...
        private void PlotChart(string runtime, string sensorid, int i)
        {
            ...
            Series r;
                ...
                dbQueryStringm = "SELECT [TIMESTAMP],[VALUE] AS Value FROM [345].[dbo].[asc_output_occupancy] WHERE EQUIPMENT_NO = '" + sensorid + "' AND [RUNTIME] = '" + runtime + "'  ORDER BY [TIMESTAMP]";

                dbQueryStringr = "SELECT [TIMESTAMP],[VALUE] AS Value FROM  [345].[dbo].[345---------000SECCNTPEOBUI---VAL001] WHERE EQUIPMENT_NO = '" + sensorid + "' AND [TIMESTAMP] >= DATEADD(hour,-12,(SELECT MIN([TIMESTAMP]) FROM [345].[dbo].[asc_output_occupancy] WHERE EQUIPMENT_NO = '" + sensorid + "' AND [RUNTIME] = '" + runtime + "')) AND [TIMESTAMP] <= (SELECT MAX([TIMESTAMP]) FROM [345].[dbo].[asc_output_occupancy] WHERE EQUIPMENT_NO = '" + sensorid + "' AND [RUNTIME] = '" + runtime + "')  ORDER BY [TIMESTAMP]";
