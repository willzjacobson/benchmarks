...
class Weather_Covariates:
	...
	def __init__(self, keys, weather_obj, learn_from_days, options, lgr):
		...
		# self.columns = ['bldg_oper_min_temp', 'bldg_oper_max_temp',
			# 'bldg_oper_avg_temp', 'humidex_prev_day',
			# 'humidex_diff_prev_day', 'humidex_prev_wk',
			# 'humidex_diff_prev_wk', 'humidex_sim_day',
			# 'humidex_diff_sim_day']
		self.columns = ['bldg_oper_min_temp', 'bldg_oper_max_temp',
			'bldg_oper_avg_temp']
		for day_suffix in ['_prev_day', '_prev_wk', '_sim_day']:
		...
		for ts in self.keys:
			...
			# for tmp_dt in self.learn_from_days[dt]:
				# if tmp_dt:
					...
					# humidex_tmp_ts = self.weather_obj.humidex_regularized[tmp_ts]
					# humidex_diff_tmp_ts = humidex_tmp_ts - humidex
					# data.extend([humidex_tmp_ts, humidex_diff_tmp_ts])
			...
			for tmp_dt in self.learn_from_days[dt]:
				#humidex_tmp_ts = self.weather_obj.humidex_regularized[tmp_ts]
				# humidex_tmp_ts = humidex_data[tmp_ts]
				# humidex_diff_tmp_ts = humidex_tmp_ts - humidex
				# data.extend([humidex_tmp_ts, humidex_diff_tmp_ts])
				if tmp_dt:
					tmp_ts = datetime.datetime.combine(tmp_dt, tm)
