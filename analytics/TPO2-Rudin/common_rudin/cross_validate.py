...
def save_grid_srch_results(results, run_ts, floor, quadrant, options, lgr):
	...
	insert_stmt = """
		INSERT INTO [%s].dbo.[%s]
		(Run_DateTime, Hour, Floor, Quadrant, c, gamma, avg_error)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	""" % (options.results_db, options.xval_results_table)
	...
	for hour, result in enumerate(results):
		for result_row in result.iteritems():
			...
			c, gamma = params
			insert_seq.append((run_ts, hour, floor, quadrant, c, gamma,
...
def find_latest_training_files(floor, quadrant, options, lgr, directory='.'):
	""" find most recent training covariate csv files in the directory
	...
	TRAIN_FILENAME_FORMAT = re.compile(
		r'%s' % options.train_covariate_filename_fmt % (options.building_id,
			floor, quadrant))
...
def add_config_options(mod_optgroup, mod_cfggroup):
	""" add configuration settings to be read from config file """

	pass
	# train_covariate_filename_fmt
	# mod_optgroup.add_option('--train_covariate_filename_fmt',
		# help = 'training covariates file name format')
	# mod_cfggroup.add_option('train_covariate_filename_fmt', keys=_module)

	# data_granularity
	# mod_optgroup.add_option('--data_granularity', type='int',
		# help='training data granularity in minutes')
	# mod_cfggroup.add_option('data_granularity', keys=_module)

	# results_db
	# mod_optgroup.add_option('--results_db',
		# help = 'results database')
	# mod_cfggroup.add_option('results_db', keys=_module)

	# results_db_server
	# mod_optgroup.add_option('--results_db_server',
		# help = 'results database server')
	# mod_cfggroup.add_option('results_db_server', keys=_module)

	# results_table
	# mod_optgroup.add_option('--results_table',
		# help = 'results table')
	# mod_cfggroup.add_option('results_table', keys=_module)
	...
	for bldg_idx, building_id in enumerate(building_ids):
		...
		for floor_idx, bldg_floor in enumerate(bldg_floors):
			...
			try:
				...
				latest_trn_files = find_latest_training_files(bldg_floor,
					floor_quadrant, options, lgr, options.temp_dir)
				...
				for hour_idx in range(0, 24):
					...
					try:
						all_results.append(find_optimal_params(train_covariates,
							options.forecast_granularity, ONE_WEEK, ONE_WEEK, options, lgr))
					except Exception, e:
						lgr.critical('cross validation failed for hour %d: %s' % (
				...
				# save results to db
				save_grid_srch_results(all_results, run_ts, bldg_floor,
					floor_quadrant, options, lgr)

			except Exception, e:
				lgr.critical('cross validation failed: %s' % traceback.format_exc())
