...
class Historical_Covariates:
	...
	def __init__(self, keys, space_temp_obj, weather_obj, hour, model_type,
		...
		self.columns = ['day_ago_%s' % model_type,
			...
			'prev_wk_hour_avg_%s' % model_type, 'sim_day_%s' % model_type,
			'sim_day_avg_%s' % model_type, 'sim_day_hour_avg_%s' % model_type]
	...
	def find_day_ago_ts(self, ts, dt, wk_day, is_hol, hol_pct, wk_ago_ts):
		"""
			...
			case 2: today is a holiday: If previous similar-business-day was
				a holiday, just use the day a week ago
		"""

		day_ago_ts = ts - self.ONE_DAY
		is_hol_day_ago, _ = self.holidays_obj.is_holiday(
				day_ago_ts)

		if wk_day not in [1, 5, 6, 7]:
			if is_hol or is_hol_day_ago:
		...
		for i, row in enumerate(sim_day_cache):
			...
			if not is_hol:
				...
				if is_hol_tmp:
					...
					days_diff = abs(tmp_dt.toordinal() - ordinal)

					# if the holiday occupancy is close enough
					# and the holiday lies around the same time of the year, use it
					if hol_occ_pct_diff <= 0.05 and days_diff in range(350, 380):
						if tmp_ts in self.space_temp_obj.data:
							return tmp_ts
					else:
	...
	def gen_covariates_new(self):
		...
		for ts in self.keys:
			...
			row = []
			learn_from_days[dt] = []
			for tmp_ts in [day_ago_ts, week_ago_ts, sim_day_ts]:
