...
_module = 'score_prediction_electric_tenant'

__doc__ = """ score electric tenant predictions
				using observed data
		  """
...
from electric.electric_tenant import Electric_Tenant

# STEAM = 'steam_demand'
ELECTRIC = 'electric_load'
MODEL_TYPES = [ELECTRIC]

def get_observed_data(tenant_id, options, lgr):
	""" """

	now = datetime.datetime.now() #datetime.datetime(2099, 1, 1)

	data_obj = Electric_Tenant(lgr, options, now, tenant_id)
...
def get_prediction_data(cursr, tenant_id, options, lgr):
...
def main(argv):
	...
	# get building ids
	tenant_ids = utils.parse_value_list(options.tenant_ids)
	...
	for tenant_idx, tenant_id in enumerate(tenant_ids):
		...
		options = utils.load_tenant_config(tenant_id, oparser, cparser, argv,
		...
		# fetch observed data
		observed_data = get_observed_data(tenant_id, options, lgr)

		# fetch predictions
		prediction_data = get_prediction_data(cursr,
			tenant_id, options, lgr)
