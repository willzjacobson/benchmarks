...
_module = 'cross_validate_occupancy'
...
def save_grid_srch_results(results, run_ts, options, lgr):
	...
	insert_stmt = """
		...
		VALUES (?, ?, ?, ?, ?)
	""" % (options.results_db, options.xval_results_table_occupancy)
...
def find_latest_training_files(options, lgr, directory='.'):
	...
	TRAIN_FILENAME_FORMAT = re.compile(r'%s' %
		options.train_covariate_filename_fmt_occupancy % options.building_id)
