...
from get_forecast import Occupancy_Forecast_Lite
...
def save_trigger(ts, trig_type, run_ts, options, lgr):
	...
	insert_stmt = """
		...
		VALUES (?, ?, ?)
	""" % (options.results_db, 'occupancy_forecast_vfd_triggers')#options.occ_vfd_trigger_table) TODO
...
def save_deltas(keys, obs, run_ts, options, lgr):
	...
	insert_stmt = """
		...
		VALUES (?, ?, ?)
	""" % (options.results_db, 'occupancy_forecast_vfd_trigger_deltas')#options.occ_vfd_trigger_table) TODO
...
def merge_obs(old_obs, new_obs, holiday_data, options, lgr):
	...
	if options.debug:
		utils.write_dict_to_csv(old_obs, '%s_merged_obs_forecast.csv' % options.building_id, None)
	...
	for inflection_pt in reversed(inflection_pts):
		...
		if (not inf_trig and occ >= MIN_TRIG_OCC and delta >= trigger_delta):
			...
			if (not last_trig_ts or
				...
				(last_trig_type == trig_type and ts - last_trig_ts > EIGHTEEN_HOURS) or
...
def reload_state(building_id, options ,lgr):
	...
	obs = OrderedDict([])
	obs = utils.unpickler('%s_occupancy_forecast_data.p' % building_id, obs,
		lgr, options)
	
	infl_pts = []
	infl_pts = utils.unpickler('%s_forecast_inflection_pts.p' % building_id,
	...
	trigger_pts = utils.unpickler('%s_forecast_trigger_pts.p' % building_id,
	...
	max_occ = utils.unpickler('%s_forecast_max_occ.p' % building_id,
		max_occ, lgr, options)
	if options.debug:
	...
	holiday_data = {}
	holiday_data = utils.unpickler('%s_holiday_data_forecast.p' % building_id,
...
def save_state(obs, inflection_pts, trigger_pts, max_occ, holiday_data,
	...
	utils.pickler('%s_occupancy_forecast_data.p' % building_id, obs_trim, lgr, options)
	utils.pickler('%s_forecast_inflection_pts.p' % building_id, infl_pts_trim,
		lgr, options)
	utils.pickler('%s_forecast_trigger_pts.p' % building_id, trig_pts_trim,
		lgr, options)
	utils.pickler('%s_forecast_max_occ.p' % building_id, max_occ,
		lgr, options)
	utils.pickler('%s_holiday_data_forecast.p' % building_id, holiday_data,
		lgr, options)
...
def main(argv):
	...
	for bldg_idx, building_id in enumerate(building_ids):
		...
		try:
			...
			occ_obj = Occupancy_Forecast_Lite(lgr, options, last_obs_ts, building_id)
