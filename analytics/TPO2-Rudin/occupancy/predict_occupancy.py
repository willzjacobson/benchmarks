...
def main(argv):
	...
	for bldg_idx, building_id in enumerate(building_ids):
		...
		# read inputs
		occ_obj = Occupancy(lgr, options, forecast_start_ts, building_id)
		
		# compute similar weather days
		weather_obj.compute_similar_weather_day_cache(occ_obj.data.keys(),
			model_type)
		...
		for hour_idx in range(0, 24):
			...
			try:
				...
				opt_c, opt_gamma = utils.get_optimal_model_params_bldg(hour_idx,
					forecast_start_ts, options.xval_results_table_occupancy,
					options, lgr)
				...
				utils.save_prediction_steam_electric(run_ts, hour_idx, p_labels,
					test_covr_obj.actual_keys, options.results_table_occupancy,
				...
				try:
					...
					failover = Failover(hour_idx, forecast_start_ts,
						run_ts, model_type, options.sign_prog_occupancy,
						options, lgr)
					p_labels_f = failover.compute_failover_forecast(
						occ_obj, weather_obj, holidays_obj)

					# save failover predictions
					utils.save_prediction_steam_electric(run_ts, hour_idx,
						p_labels_f, failover.keys, options.results_table_occupancy,
						None, options, lgr)
				except Exception, e:
			...
			try:
				if failure_count:
					...
					failover = Failover(None, forecast_start_ts, run_ts,
						model_type, options.sign_prog_occupancy, options, lgr)
