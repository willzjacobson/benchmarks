...
""" predict electric demand of tenants based on
	past observations and other covariates
"""

__version__ = '$Id'
__author__ = 'agagneja@ccls.columbia.edu'
_module = 'predict_electric_usage_tenant'
...
from electric_tenant import Electric_Tenant
...
def main(argv):
	...
	#weather_station_ids = utils.parse_value_list(options.tenant_weather_station_ids)

	model_type = 'electric_demand_tenant'
	...
	for tenant_idx, tenant_id in enumerate(tenant_ids):
		...
		electric_obj = Electric_Tenant(lgr, options, forecast_start_ts, tenant_id)
			...
			try:
				...
				try:
					...
					failover = Failover(hour_idx, forecast_start_ts, run_ts,
						model_type, options.sign_prog, options, lgr)
