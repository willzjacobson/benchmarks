...
_module = 'cross_validate_electric_tenant'
...
def save_grid_srch_results(results, run_ts, options, lgr):
	...
	cnxn, cursr = connect(options.db_driver, options.db_user, options.db_pwd,
	...
	insert_stmt = """
		...
		VALUES (?, ?, ?, ?, ?)
	""" % (options.results_db, options.xval_results_table_electric)
...
def find_latest_training_files(options, lgr, directory='.'):
	...
	TRAIN_FILENAME_FORMAT = re.compile(r'%s' %
		options.train_covariate_filename_fmt_electric % options.tenant_id)
