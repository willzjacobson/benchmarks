...
def _get_weather(h5file_name, history_name, forecast_name, gran):
    ...
    with pd.HDFStore(h5file_name) as store:
        munged_history = larkin.weather.wund.history_munge(store[history_name],
                                                    "%dmin" % gran)

        munged_forecast = larkin.weather.wund.forecast_munge(store[forecast_name],
                                                      "%dmin" % gran)
        # cov, "%dmin" % gran)
...
def find_lowest_electric_usage(date_scores, electric_ts, n, debug):
    ...
    for i, dt in enumerate(dates):
        ...
        if score:
            ...
            day_elec_ts = larkin.shared.utils.drop_series_ix_date(
                    larkin.shared.utils.get_dt_tseries(dt, electric_ts))

            # compute total and incremental AUC
            x = list(map(lambda y: y.hour + y.minute / 60.0 + y.second / 3600.0,
                         day_elec_ts.index))
            incr_auc, auc = larkin.benchmarks.utils.incremental_trapz(
...
def _find_benchmark(base_dt, occ_ts, wetbulb_ts, electric_ts, gran, debug):
    ...
    elec_avlblty = larkin.benchmarks.utils.get_data_availability_dates(electric_ts,
                                                                       gran)
    ...
    return larkin.benchmarks.utils.find_lowest_auc_day(occ_scores, electric_ts, 5,
                                                       debug)
...
def process_building(building_id, host, port, db_name, username, password,
    ...
    # get occupancy data
    occ_ts = larkin.ts_proc.utils.get_occupancy_ts(host, port, db_name, username,
                                                   password, source,
    ...
    # query electric data
    elec_ts = larkin.ts_proc.utils.get_electric_ts(host, port, db_name, username,
                                                   password, source,
                                                   collection_name,
