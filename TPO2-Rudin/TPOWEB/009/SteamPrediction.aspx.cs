...
namespace SmartBuilding
{
    ...
    /// </summary>
    public partial class diboss_009_SteamPrediction : System.Web.UI.Page
    {
        ...
        private void FillData()
        {
            ...
            string sensorid = "";
            string sensorname = "";
            string querySensor = "SELECT DISTINCT TABLE_NAME, REPLACE(TABLE_NAME, '009_Prediction_', '') AS SHORT_NAME FROM Prediction_CONFIG WHERE TABLE_NAME LIKE '%009_Prediction_STEAM%' ORDER BY TABLE_NAME";
            ...
            dbQueryStringa = "SELECT Value AS Actual, TIMESTAMP FROM [355].dbo.[355---------009BMSSTEMET------VAL001] where (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";

            dbQueryStringm = "SELECT [Prediction_Date] AS TIMESTAMP, [RMSE] AS Value FROM [355].[dbo].[355---------000TPOFORSTECON001---001_Stats] WHERE ([Prediction_Date] > '" + fromDate.ToString() + "') AND (Prediction_Date < '" + toDate.ToString() + "') ORDER BY [Prediction_Date]";
            ...
            dbQueryStartUpTime = "SELECT 30 AS Value, TIMESTAMP FROM [009_Prediction_Start_Up_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
            ...
            dbQueryRampDownTime = "SELECT 0 AS Value, TIMESTAMP FROM [009_Prediction_Ramp_Down_Time] WHERE (TIMESTAMP > '" + fromDate.ToString() + "') AND (TIMESTAMP < '" + toDate.ToString() + "') ORDER BY TIMESTAMP";
