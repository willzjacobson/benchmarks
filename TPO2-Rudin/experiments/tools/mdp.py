...
class DatabaseInterface:
	...
	def get_bms_series(self, tables, _from, _to, _keys=None):
...
class DataFrame:
	...
	def process(self, granularity):
		...
		# ag2818: should the base be replaced by current
		while True:
		...
		for [pointname, ts, values] in ud:
			...
			#function = interpolate.interp1d(x, values, kind='nearest')
...
class StateAction:
	...
	def cluster(self, dimensions, visual=False):
		...
		for i in xrange(len(pointnames)):
			if "FANLCP" in pointnames[i] or "WATSEC" in pointnames[i] or "HVAPFASAT" in pointnames[i]:
		...
		#Max_StateSet_Cardinality = 40
		#Min_StateSet_Cardinality = 20
		#print "Adjusting state set size..."
		#a_gmm, action_size = modelSelection(X, Max_StateSet_Cardinality, Min_StateSet_Cardinality) 
		#print "State set size set to {0}".format(action_size)


		Max_ActionSet_Cardinality = 20
		Min_ActionSet_Cardinality = 2
		print "Adjusting action set size..."
		a_gmm, action_size = modelSelection(action_matrix, Max_ActionSet_Cardinality, Min_ActionSet_Cardinality) 
		print "Action set size set to {0}".format(action_size)

		self.actions_model = a_gmm

		modes = gmm.predict(X)
		modes = a_gmm.predict(action_matrix)
	...
	def transition(self):
		...
		for action in trans_prob_matrix:
			for i in xrange(len(trans_prob_matrix[action])):
				if numpy.sum(trans_prob_matrix[action][i]) < 0.01:
					#trans_prob_matrix[action][i][i] = 1
					continue
		...
		for (ax, action) in zip(axes.flat, trans_prob_matrix):
			ax.imshow(trans_prob_matrix[action], interpolation='none')
		plt.show()
	...
	def cost(self, minute):
		...
		for pointname in self.state_pointnames: 
			...
			elif "BMSSTEM" in pointname or "BMSELEMET" in pointname:
				a.append(1)
				b.append(0)
		...
		for i in xrange(len(centroids)):
			...
			temp_vector = numpy.power(numpy.abs((mean - 73)*b), 3)
			temp_factor = numpy.sum(temp_vector)

			self.costs[i, 0] = energy_vector*1 + temp_factor*0
			self.costs[i, 1] = energy_vector*1 + temp_factor*0.2

		return self.costs.T[alpha]


	def scenario(self, last_state, action_sequence, last_dt, new=True, final=True):
		...
		_color = numpy.random.rand(3)
		for y in D.T:
			self.ax.plot(dt_list, y, color=_color)

		self.ax.plot(dt_list, rewards, '--', color=_color)
	...
	def backward_induction(self, state_index):
		...
		for s in state_set:	
			...
			r = int(numpy.random.rand()*(len(self.actions)-24*4*2))

			while self.dataframe.timestamps[r].hour != 0:
				r += 1

			#print self.actions[r:r+24*4]
			print action_sequence

			pol = self.actions[r:r+24*4]
			#self.scenario(self.states[state_index], pol, self.dataframe.timestamps[state_index], new=True, final=False)
			for i in range(0, 10):
				r = int(numpy.random.rand()*(len(self.actions)-24*4*2))
				...
				#self.scenario(self.states[state_index], self.actions[r:r+24*4], self.dataframe.timestamps[state_index], new=False, final=False)
			
			#self.scenario(self.states[state_index], self.actions[r:r+24*4], self.dataframe.timestamps[state_index], new=False, final=True)
...
def modelSelection(X, maxParam, minParam):
	
	modelBag = []
	BIC = []
	for i in range(minParam, maxParam + 1):
...
s = []
# Actions
s += d.get_bms_series("345---------001BMSHVAFANLCP---VAL001", '2014-08-01', '2014-11-01')
#s += d.get_bms_series("345---------001BMSHVAFANSSP---VAL001", '2014-08-01', '2014-11-01')
s += d.get_bms_series("345---------001BMSWATSECPUM---VAL001", '2014-08-01', '2014-11-01')
s += d.get_bms_series("345---------001BMSHVAPFASAT---SPV001", '2014-08-01', '2014-11-01')

# States
s += d.get_bms_series("345---------001BMSHVATEMSPA---VAL001", '2014-08-01', '2014-11-01', _keys=[('F02', 'CNW'), ('F04', 'CNW'), ('F04', 'CSE'), ('F05', 'CSE'), ('F13', 'CSW'), ('F18', 'CSE'), ('F20', 'CSW'), ('F24', 'CSE'), ('F32', 'CNW'), ('F38', 'CNW'), ('F40', 'CNE')])
s += d.get_bms_series("345---------001BMSSTEMET------MVA001", '2014-08-01', '2014-11-01')
s += d.get_bms_series("345---------001BMSELEMET------VAL001", '2014-08-01', '2014-11-01')
#s += d.get_bms_series("345---------000SECCNTPEOBUI---VAL001", '2014-08-01', '2014-11-01')


d = DatabaseInterface("anderson.ldeo.columbia.edu", "Weather", "Hooshmand", "Breakit68")
#s += d.get_generic_series("Observations_History", "Date", ["TempA", "DewPointA", "Humidity", "PressureA"], '2014-08-01', '2014-11-01')
