...
def _find_benchmark(base_dt, occ_ts, wetbulb_ts, obs_ts, gran, timezone,
    ...
    steam_avlblty = nikral.benchmarks.utils.get_data_availability_dates(obs_ts,
                                                                       gran)
    ...
    return nikral.benchmarks.utils.find_lowest_auc_day(occ_scores, obs_ts, 2,
                                                       timezone, debug)
...
def process_building(building, host, port, db_name, username, password,
    ...
    # query steam data
    steam_ts = nikral.ts_proc.utils.get_parsed_ts_new_schema(host, port,
                                                             db_name, username,
                                                             password, source,
                                                             collection_name,
                                                             building,
                                                             'TotalInstant')
    steam_ts = nikral.benchmarks.utils.align_idx(steam_ts, gran_int).tz_convert(
        target_tzone)
    nikral.shared.utils.debug_msg(debug, "steam: %s" % steam_ts)
