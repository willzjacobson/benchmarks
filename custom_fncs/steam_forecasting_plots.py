...
park_ts_filter = park_ts[np.logical_and(park_ts.index.weekday == 0, park_ts < park_ts.shift(1))]
print(park_ts_filter)

'''
Observe that almost all the data is for 15 minutes after midnight. Hence, it is
reasonable to exclude all other times from our analysis, and to fill in gaps via interpolation.
'''

park_ts_filter_15 = park_ts_filter[park_ts_filter.index.minute == 15].interpolate()
...
print(arima.ARIMA(park_ts_filter_15[park_ts_filter_15.between(
    basic_stats_15['5%'], basic_stats_15['95%'])],
...
# TODO Get feedback from Gene about why this is occurring. Believe it to be a BMS error.
